...
function RwLogin(){
  ...
  const login = e => {
    ...
    axios.post('http://localhost:3001/rwlogin', data).then(
      (response) => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3001/rwlogin').then(
      (response) => {
    ...
    <div className="bg-light">
      <Container className="justify-content-center card-div">
        <Card className="home-card">
          <Card.Body>
            <Card.Title><h2>Admin Panel</h2></Card.Title>
      ...
      {auth && <Redirect exact to="/complains" />}
      {islogged && <Redirect exact to="/complains" />}
    </div>
  );
}
